...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (int i = 0; i < row_1 / 8; i++)
	{
		MPI_Recv(&(proc4_res[i][0]), column_2, MPI_LONG_DOUBLE, 0, tagA4, MPI_COMM_WORLD, &status);
		MPI_Recv(&(proc2_res[i][0]), column_2, MPI_LONG_DOUBLE, 0, tagA2, MPI_COMM_WORLD, &status);
	}
	...
	for (int i = 0; i < (row_1 / 8); i++)
	{
		...
		MPI_Send(&(proc2_res[i][0]), column_2, MPI_LONG_DOUBLE, 2, tagA2, MPI_COMM_WORLD);
		MPI_Send(&(proc7_res[i][0]), column_2, MPI_LONG_DOUBLE, 2, tagA7, MPI_COMM_WORLD);
	}
	cout << "Send proc4_res,proc2_res, proc7_res from processor with rank 7 to processor with rank 6" << endl;
}