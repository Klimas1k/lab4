...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (int i = 0; i < row_1 / 8; i++)
	{
		...
		MPI_Recv(&(proc2_res[i][0]), column_2, MPI_LONG_DOUBLE, 4, tagA2, MPI_COMM_WORLD, &status);
		MPI_Recv(&(proc7_res[i][0]), column_2, MPI_LONG_DOUBLE, 4, tagA7, MPI_COMM_WORLD, &status);
		MPI_Recv(&(proc3_res[i][0]), column_2, MPI_LONG_DOUBLE, 4, tagA3, MPI_COMM_WORLD, &status);
		MPI_Recv(&(proc1_res[i][0]), column_2, MPI_LONG_DOUBLE, 4, tagA1, MPI_COMM_WORLD, &status);
	}

	for (int i = 0; i < (row_1 / 8); i++)
	{
		MPI_Send(&(proc4_res[i][0]), column_2, MPI_LONG_DOUBLE, 7, tagA4, MPI_COMM_WORLD);
		MPI_Send(&(proc2_res[i][0]), column_2, MPI_LONG_DOUBLE, 7, tagA2, MPI_COMM_WORLD);
		MPI_Send(&(proc7_res[i][0]), column_2, MPI_LONG_DOUBLE, 7, tagA7, MPI_COMM_WORLD);
		MPI_Send(&(proc3_res[i][0]), column_2, MPI_LONG_DOUBLE, 7, tagA3, MPI_COMM_WORLD);
		MPI_Send(&(proc1_res[i][0]), column_2, MPI_LONG_DOUBLE, 7, tagA1, MPI_COMM_WORLD);
		MPI_Send(&(proc6_res[i][0]), column_2, MPI_LONG_DOUBLE, 7, tagA6, MPI_COMM_WORLD);
	}
	cout << "Send pro4_res,pro2_res,pro7_res,pro3_res, pro1_res, proc6_res from processor with rank 6 to processor with rank 7" << endl;
}