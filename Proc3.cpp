...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (int i = 0; i < row_1 / 8; i++)
	{
		...
		MPI_Recv(&(proc2_res[i][0]), column_2, MPI_LONG_DOUBLE, 3, tagA2, MPI_COMM_WORLD, &status);
		MPI_Recv(&(proc7_res[i][0]), column_2, MPI_LONG_DOUBLE, 3, tagA7, MPI_COMM_WORLD, &status);
	}

	for (int i = 0; i < (row_1 / 8); i++)
	{
		MPI_Send(&(proc4_res[i][0]), column_2, MPI_LONG_DOUBLE, 6, tagA4, MPI_COMM_WORLD);
		MPI_Send(&(proc2_res[i][0]), column_2, MPI_LONG_DOUBLE, 6, tagA2, MPI_COMM_WORLD);
		MPI_Send(&(proc7_res[i][0]), column_2, MPI_LONG_DOUBLE, 6, tagA7, MPI_COMM_WORLD);
		MPI_Send(&(proc3_res[i][0]), column_2, MPI_LONG_DOUBLE, 6, tagA3, MPI_COMM_WORLD);
	}
	cout << "Send proc3_res, proc4_res, pro2_res, proc7_res from processor with rank 3 to processor with rank 6" << endl;
}